        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-surface border border-slate-200 rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex flex-col items-center text-center space-y-4">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-slate-900 tracking-tight">{user.firstName} {user.lastName}</h1>
                        ...
                        <div className="mt-8 pt-8 border-t border-slate-100 space-y-4">
                            <div className="flex items-center gap-4">
                                <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center border border-slate-100">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="space-y-0.5">
                                    ...
                                    <p className="text-sm text-slate-700 font-medium">{new Date(user.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-3">
                            ...
                            <button className="flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-surface text-slate-500 border border-slate-200 hover:bg-slate-50 text-[10px] font-bold uppercase tracking-widest transition-all">
                                <AlertCircle size={14} />
                                Reset PIN
                            </button>
                        </div>
                        ...
                        <div className="mt-8 pt-8 border-t border-slate-100 space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input 
                                            ...
                                            className="w-full bg-surface border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:border-emerald-500/50 transition-all"
                                ...
                                <button
                                    ...
                                    disabled={processing}
                                    className="w-full h-12 bg-surface border border-slate-200 rounded-2xl text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] hover:bg-slate-50 hover:text-slate-900 transition-all flex items-center justify-center gap-2"
                                >
                                    {processing ? <Loader2 className="animate-spin" size={16} /> : "Synchronize Overrides"}
                                </button>
                                <p className="text-[8px] text-slate-600 font-medium leading-relaxed text-center px-4">
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-100 space-y-4">
                            ...
                            <div className="space-y-3">
                                <select 
                                    ...
                                    disabled={processing}
                                    className="w-full bg-surface border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all cursor-pointer disabled:opacity-50"
                                >
                ...
                <div className="lg:col-span-8 space-y-6">
                    <div className="grid sm:grid-cols-2 gap-6">
                        <div className="bg-surface border border-slate-200 rounded-2xl p-5 space-y-4">
                            ...
                            <div>
                                <h3 className="text-3xl font-bold text-slate-900 tracking-tight">{currency}{(user.balance || 0).toLocaleString()}</h3>
                    ...
                    <div className="bg-surface border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                            <h3 className="text-lg font-bold text-slate-900 tracking-tight">Ledger History</h3>
                            <button 
                                onClick={() => navigate(`/admin/transactions?userId=${user._id}`)}
                                className="text-emerald-600 font-bold text-[10px] uppercase tracking-widest hover:underline"
                            >
                                Full Statement
                            </button>
                        </div>
                            ...
                            <table className="w-full">
                                <thead className="bg-slate-50/80">
                                    <tr className="border-b border-slate-100">
                                        <th className="text-left px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">Transaction</th>
                                        <th className="text-left px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">Amount</th>
                                        <th className="text-left px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">Node</th>
                                        <th className="text-right px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">Status</th>
                                    </tr>
                                </thead>
                                            ...
                                            <tr key={i} className="group hover:bg-slate-50/60 transition-colors">
                                                <td className="py-4 px-6">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-8 h-8 rounded-lg ${config.bg} ${config.color} flex items-center justify-center border border-slate-100`}>
                                                            {config.icon}
                                                        </div>
                                                        <div className="space-y-0.5">
                                                            <p className="text-sm font-bold text-slate-700">{config.label} {tx.service ? `- ${tx.service}` : ''}</p>
                                                ...
                                                <td className="py-4 px-6 text-right">
                                                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-widest ${
                                                        tx.status === 'success' ? 'text-emerald-600 bg-emerald-500/5' :
                                                        tx.status === 'failed' ? 'text-rose-600 bg-rose-500/5' :
