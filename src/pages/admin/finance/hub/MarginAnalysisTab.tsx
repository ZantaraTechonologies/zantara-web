    ...
    return (
        <div className="space-y-5 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-surface p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-slate-100">
                            <div><p className="text-slate-500 font-black uppercase text-[9px] mb-2">Total Sales Revenue</p><p className="text-slate-900 font-black text-2xl">₦{(totalRevenue || 0).toLocaleString()}</p></div>
                            <div><p className="text-slate-500 font-black uppercase text-[9px] mb-2">Service Inventory Cost</p><p className="text-slate-500 font-black text-2xl">₦{(totalCost || 0).toLocaleString()}</p></div>
                        </div>
                    </div>
                </div>

                <div className="bg-surface p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between group">
                    <div>
                        ...
                        <h3 className="font-black text-slate-900 text-2xl tracking-tight">Margin Health</h3>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-3 ml-2">
                    ...
                    <h3 className="font-black text-slate-900 tracking-widest uppercase text-[10px]">Categorical Profit Breakdown</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {breakdown?.map((item: any, idx: number) => (
                        <div key={idx} className="bg-surface p-6 rounded-3xl border border-slate-100 group hover:border-emerald-500/20 transition-all relative overflow-hidden">
                            <div className="flex items-center justify-between mb-8">
                                <div className="w-12 h-12 rounded-2xl bg-slate-100 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:text-emerald-400">{getCategoryIcon(item._id)}</div>
                                <div className="text-right">
                                    <p className="text-2xl font-black text-slate-900 tracking-tighter">₦{item.profit?.toLocaleString()}</p>
