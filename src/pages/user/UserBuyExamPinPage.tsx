...
const UserBuyExamPinPage: React.FC = () => {
    ...
    const handleConfirm = async (pin: string) => {
        ...
        try {
            ...
            toast.error(msg);
            navigate('/app/services/status', { state: { status: 'error', message: msg, transaction: { service: serviceTitle, amount: purchaseAmount, target: selectedBody.name, timestamp: new Date().toLocaleTimeString() } } });
        } finally {
            setLoading(false);
        }
    };
