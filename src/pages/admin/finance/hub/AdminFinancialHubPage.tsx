        ...
        <div className="space-y-6 pb-20 animate-in fade-in duration-700">
            {showNotice && (
                <div className="bg-indigo-500/10 border border-indigo-500/20 p-4 rounded-3xl flex items-center justify-between gap-4 animate-in slide-in-from-top-4 duration-500">
                    ...
                    <button onClick={() => setShowNotice(false)} className="text-slate-500 hover:text-slate-900 p-2">
                        <X size={16} />
                    </button>
                </div>
            )}
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 bg-surface border border-slate-100 p-1.5 rounded-2xl shadow-sm backdrop-blur-md">
                        ...
                        {(['this-month', 'last-30-days', 'all-time', 'custom'] as const).map((r) => (
                            <button
                                ...
                                className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                                    period === r ? "bg-indigo-500 text-white shadow-lg shadow-indigo-500/20" : "text-slate-500 hover:text-slate-900"
                                }`}
                            >
                                {r.replace(/-/g, ' ')}
                            </button>
                    ...
                    {period === 'custom' && (
                        <div className="flex items-center gap-2 animate-in slide-in-from-left-2 duration-300 px-2">
                            <input 
                                type="date" 
                                className="bg-surface border border-slate-200 rounded-xl px-3 py-2 text-slate-900 text-[10px] font-black uppercase outline-none color-scheme-light"
                ...
                {tabs.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={`p-6 rounded-3xl border transition-all text-left flex items-start gap-4 group relative overflow-hidden ${
                                isActive 
                        ...
                        >
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-colors ${
                                isActive ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-500 group-hover:text-slate-900"
                            }`}>
                                <tab.icon size={22} />
                            </div>
                            <div className="relative z-10">
                                ...
                                <h3 className={`text-sm font-black tracking-tight ${isActive ? "text-slate-950" : "text-slate-900"}`}>{tab.label}</h3>
