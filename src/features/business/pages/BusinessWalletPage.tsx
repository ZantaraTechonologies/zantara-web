...
const BusinessWalletPage: React.FC = () => {
    ...
    // Calculate main node balance from backend wallet if available
    const mainNode = wallet ? {
        providerName: 'Primary VTU Gateway',
        balance: wallet.platformBalance || 0,
        usage: summary.totalCost || 0,
        status: 'Stable',
        lastSync: 'Just Now'
    } : null;
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    Array(1).fill(0).map((_, i) => <CardSkeleton key={i} />)
                ) : !mainNode ? (
                    ...
                    <div className="bg-white/5 border border-white/5 rounded-3xl p-8 space-y-6 hover:border-emerald-500/20 transition-all group relative overflow-hidden">
