        ...
        <div className="max-w-4xl mx-auto p-4 sm:p-8 lg:p-12 space-y-12 animate-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="flex items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Linked Hubs</h1>
                        <p className="text-slate-500 font-medium">Manage your verified withdrawal destinations.</p>
                    </div>
                </div>
                {!isAdding && (
                    <button 
                        onClick={() => setIsAdding(true)}
                        className="hidden sm:flex items-center gap-2 bg-emerald-400 hover:bg-emerald-500 text-slate-950 px-6 py-3 rounded-xl font-extrabold transition-all shadow-lg shadow-emerald-500/20"
                    >
                        <Plus size={18} />
            ...
            {isAdding ? (
                <div className="bg-white border border-slate-50 rounded-[3rem] p-10 sm:p-14 space-y-10 shadow-sm animate-in zoom-in-95 duration-500">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-500">
                                <Building2 size={24} />
                            </div>
                            <h2 className="text-xl font-black text-slate-900">Configure New Node</h2>
                        </div>
                    ...
                    <form onSubmit={handleAdd} className="space-y-8">
                        ...
                        <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Account Holder Name (Verified)</label>
                            <input 
                                required
                                type="text" 
                                value={accountName}
                        ...
                        <button 
                            type="submit"
                            disabled={loading}
                            className="w-full bg-emerald-400 text-slate-950 py-6 rounded-[1.5rem] font-black uppercase tracking-widest text-[11px] hover:bg-emerald-500 transition-all shadow-2xl shadow-emerald-500/20 disabled:opacity-30"
                        >
                    ...
                    {linkedAccounts.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {linkedAccounts.map((acc) => (
                                <div key={acc._id} className="bg-white border border-slate-50 p-8 rounded-[2.5rem] flex flex-col justify-between space-y-8 shadow-sm group hover:shadow-md transition-shadow">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <p className="text-sm font-black text-slate-400 tracking-widest">{acc.accountNumber.replace(/.(?=.{4})/g, '*')}</p>
                                            <span className="flex items-center gap-1.5 text-[9px] font-black text-emerald-500 bg-emerald-50 px-2 py-1 rounded-md uppercase tracking-widest">
                                    ...
                                    <div className="pt-6 border-t border-slate-50">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Holder</p>
                            ...
                            <button 
                                onClick={() => setIsAdding(true)}
                                className="border-2 border-dashed border-slate-100 rounded-[2.5rem] p-8 flex flex-col items-center justify-center gap-4 hover:bg-slate-50 hover:border-emerald-200 transition-all text-slate-400 group h-full min-h-[280px]"
                            >
                                <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center group-hover:bg-emerald-50 group-hover:text-emerald-500 transition-colors">
                                    <Plus size={32} />
                                </div>
                                <span className="font-black text-xs uppercase tracking-[0.2em]">Scale Network</span>
                            </button>
                        </div>
                    ) : (
                        <div className="bg-white border border-slate-50 rounded-[3rem] p-16 text-center space-y-8 shadow-sm">
                            <div className="w-24 h-24 bg-slate-50 rounded-[2rem] flex items-center justify-center text-slate-200 mx-auto">
                                <Building2 size={48} />
                            </div>
                            <div className="space-y-2">
                                <h3 className="text-2xl font-black text-slate-900">No Settlement Hubs</h3>
                                <p className="text-slate-500 font-medium max-w-sm mx-auto">Link a verified bank account to enable professional withdrawal protocols.</p>
                            </div>
                            <button 
                                onClick={() => setIsAdding(true)}
                                className="bg-slate-950 text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-[11px] hover:bg-emerald-500 hover:text-slate-950 transition-all shadow-2xl shadow-slate-200"
                            >
